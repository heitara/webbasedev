package com.gameif.portal.entity;

import java.util.Date;

import com.gameif.common.entity.BaseEntity;

public class QuestionnaireAnswer extends BaseEntity {

	/**
	 * 
	 */
	private static final long serialVersionUID = -8441425535840777806L;

	private Long memNum;
	private Integer questionNo;
	private Date answerDate;
	private Integer question1;
	private String remarks1;
	private Integer question2;
	private String remarks2;
	private Integer question3;
	private String remarks3;
	private Integer question4;
	private String remarks4;
	private Integer question5;
	private String remarks5;
	private Integer question6;
	private String remarks6;
	private Integer question7;
	private String remarks7;
	private Integer question8;
	private String remarks8;
	private Integer question9;
	private String remarks9;
	private Integer question10;
	private String remarks10;
	private Integer[] question11;
	private Integer question11Db;
	private String remarks11;
	private Integer[] question12;
	private Integer question12Db;
	private String remarks12;
	private Integer[] question13;
	private Integer question13Db;
	private String remarks13;
	private Integer[] question14;
	private Integer question14Db;
	private String remarks14;
	private Integer[] question15;
	private Integer question15Db;
	private String remarks15;
	private Integer[] question16;
	private Integer question16Db;
	private String remarks16;
	private Integer[] question17;
	private Integer question17Db;
	private String remarks17;
	private Integer[] question18;
	private Integer question18Db;
	private String remarks18;
	private Integer[] question19;
	private Integer question19Db;
	private String remarks19;
	private Integer[] question20;
	private Integer question20Db;
	private String remarks20;
	private String question21;
	private String question22;
	private String question23;
	private String question24;
	private String question25;

	/**
	 * @return the memNum
	 */
	public Long getMemNum() {
		return memNum;
	}

	/**
	 * @param memNum
	 *            the memNum to set
	 */
	public void setMemNum(Long memNum) {
		this.memNum = memNum;
	}

	/**
	 * @return the questionNo
	 */
	public Integer getQuestionNo() {
		return questionNo;
	}

	/**
	 * @param questionNo
	 *            the questionNo to set
	 */
	public void setQuestionNo(Integer questionNo) {
		this.questionNo = questionNo;
	}

	/**
	 * @return the answerDate
	 */
	public Date getAnswerDate() {
		return answerDate;
	}

	/**
	 * @param answerDate
	 *            the answerDate to set
	 */
	public void setAnswerDate(Date answerDate) {
		this.answerDate = answerDate;
	}

	/**
	 * @return the question1
	 */
	public Integer getQuestion1() {
		return question1;
	}

	/**
	 * @param question1
	 *            the question1 to set
	 */
	public void setQuestion1(Integer question1) {
		this.question1 = question1;
	}

	/**
	 * @return the remarks1
	 */
	public String getRemarks1() {
		return remarks1;
	}

	/**
	 * @param remarks1
	 *            the remarks1 to set
	 */
	public void setRemarks1(String remarks1) {
		this.remarks1 = remarks1;
	}

	/**
	 * @return the question2
	 */
	public Integer getQuestion2() {
		return question2;
	}

	/**
	 * @param question2
	 *            the question2 to set
	 */
	public void setQuestion2(Integer question2) {
		this.question2 = question2;
	}

	/**
	 * @return the remarks2
	 */
	public String getRemarks2() {
		return remarks2;
	}

	/**
	 * @param remarks2
	 *            the remarks2 to set
	 */
	public void setRemarks2(String remarks2) {
		this.remarks2 = remarks2;
	}

	/**
	 * @return the question3
	 */
	public Integer getQuestion3() {
		return question3;
	}

	/**
	 * @param question3
	 *            the question3 to set
	 */
	public void setQuestion3(Integer question3) {
		this.question3 = question3;
	}

	/**
	 * @return the remarks3
	 */
	public String getRemarks3() {
		return remarks3;
	}

	/**
	 * @param remarks3
	 *            the remarks3 to set
	 */
	public void setRemarks3(String remarks3) {
		this.remarks3 = remarks3;
	}

	/**
	 * @return the question4
	 */
	public Integer getQuestion4() {
		return question4;
	}

	/**
	 * @param question4
	 *            the question4 to set
	 */
	public void setQuestion4(Integer question4) {
		this.question4 = question4;
	}

	/**
	 * @return the remarks4
	 */
	public String getRemarks4() {
		return remarks4;
	}

	/**
	 * @param remarks4
	 *            the remarks4 to set
	 */
	public void setRemarks4(String remarks4) {
		this.remarks4 = remarks4;
	}

	/**
	 * @return the question5
	 */
	public Integer getQuestion5() {
		return question5;
	}

	/**
	 * @param question5
	 *            the question5 to set
	 */
	public void setQuestion5(Integer question5) {
		this.question5 = question5;
	}

	/**
	 * @return the remarks5
	 */
	public String getRemarks5() {
		return remarks5;
	}

	/**
	 * @param remarks5
	 *            the remarks5 to set
	 */
	public void setRemarks5(String remarks5) {
		this.remarks5 = remarks5;
	}

	/**
	 * @return the question6
	 */
	public Integer getQuestion6() {
		return question6;
	}

	/**
	 * @param question6
	 *            the question6 to set
	 */
	public void setQuestion6(Integer question6) {
		this.question6 = question6;
	}

	/**
	 * @return the remarks6
	 */
	public String getRemarks6() {
		return remarks6;
	}

	/**
	 * @param remarks6
	 *            the remarks6 to set
	 */
	public void setRemarks6(String remarks6) {
		this.remarks6 = remarks6;
	}

	/**
	 * @return the question7
	 */
	public Integer getQuestion7() {
		return question7;
	}

	/**
	 * @param question7
	 *            the question7 to set
	 */
	public void setQuestion7(Integer question7) {
		this.question7 = question7;
	}

	/**
	 * @return the remarks7
	 */
	public String getRemarks7() {
		return remarks7;
	}

	/**
	 * @param remarks7
	 *            the remarks7 to set
	 */
	public void setRemarks7(String remarks7) {
		this.remarks7 = remarks7;
	}

	/**
	 * @return the question8
	 */
	public Integer getQuestion8() {
		return question8;
	}

	/**
	 * @param question8
	 *            the question8 to set
	 */
	public void setQuestion8(Integer question8) {
		this.question8 = question8;
	}

	/**
	 * @return the remarks8
	 */
	public String getRemarks8() {
		return remarks8;
	}

	/**
	 * @param remarks8
	 *            the remarks8 to set
	 */
	public void setRemarks8(String remarks8) {
		this.remarks8 = remarks8;
	}

	/**
	 * @return the question9
	 */
	public Integer getQuestion9() {
		return question9;
	}

	/**
	 * @param question9
	 *            the question9 to set
	 */
	public void setQuestion9(Integer question9) {
		this.question9 = question9;
	}

	/**
	 * @return the remarks9
	 */
	public String getRemarks9() {
		return remarks9;
	}

	/**
	 * @param remarks9
	 *            the remarks9 to set
	 */
	public void setRemarks9(String remarks9) {
		this.remarks9 = remarks9;
	}

	/**
	 * @return the question10
	 */
	public Integer getQuestion10() {
		return question10;
	}

	/**
	 * @param question10
	 *            the question10 to set
	 */
	public void setQuestion10(Integer question10) {
		this.question10 = question10;
	}

	/**
	 * @return the remarks10
	 */
	public String getRemarks10() {
		return remarks10;
	}

	/**
	 * @param remarks10
	 *            the remarks10 to set
	 */
	public void setRemarks10(String remarks10) {
		this.remarks10 = remarks10;
	}

	/**
	 * @return the question11
	 */
	public Integer[] getQuestion11() {
		return question11;
	}

	/**
	 * @param question11
	 *            the question11 to set
	 */
	public void setQuestion11(Integer[] question11) {
		this.question11 = question11;
	}

	/**
	 * @return the remarks11
	 */
	public String getRemarks11() {
		return remarks11;
	}

	/**
	 * @param remarks11
	 *            the remarks11 to set
	 */
	public void setRemarks11(String remarks11) {
		this.remarks11 = remarks11;
	}

	/**
	 * @return the question12
	 */
	public Integer[] getQuestion12() {
		return question12;
	}

	/**
	 * @param question12
	 *            the question12 to set
	 */
	public void setQuestion12(Integer[] question12) {
		this.question12 = question12;
	}

	/**
	 * @return the remarks12
	 */
	public String getRemarks12() {
		return remarks12;
	}

	/**
	 * @param remarks12
	 *            the remarks12 to set
	 */
	public void setRemarks12(String remarks12) {
		this.remarks12 = remarks12;
	}

	/**
	 * @return the question13
	 */
	public Integer[] getQuestion13() {
		return question13;
	}

	/**
	 * @param question13
	 *            the question13 to set
	 */
	public void setQuestion13(Integer[] question13) {
		this.question13 = question13;
	}

	/**
	 * @return the remarks13
	 */
	public String getRemarks13() {
		return remarks13;
	}

	/**
	 * @param remarks13
	 *            the remarks13 to set
	 */
	public void setRemarks13(String remarks13) {
		this.remarks13 = remarks13;
	}

	/**
	 * @return the question14
	 */
	public Integer[] getQuestion14() {
		return question14;
	}

	/**
	 * @param question14
	 *            the question14 to set
	 */
	public void setQuestion14(Integer[] question14) {
		this.question14 = question14;
	}

	/**
	 * @return the remarks14
	 */
	public String getRemarks14() {
		return remarks14;
	}

	/**
	 * @param remarks14
	 *            the remarks14 to set
	 */
	public void setRemarks14(String remarks14) {
		this.remarks14 = remarks14;
	}

	/**
	 * @return the question15
	 */
	public Integer[] getQuestion15() {
		return question15;
	}

	/**
	 * @param question15
	 *            the question15 to set
	 */
	public void setQuestion15(Integer[] question15) {
		this.question15 = question15;
	}

	/**
	 * @return the remarks15
	 */
	public String getRemarks15() {
		return remarks15;
	}

	/**
	 * @param remarks15
	 *            the remarks15 to set
	 */
	public void setRemarks15(String remarks15) {
		this.remarks15 = remarks15;
	}

	/**
	 * @return the question16
	 */
	public Integer[] getQuestion16() {
		return question16;
	}

	/**
	 * @param question16
	 *            the question16 to set
	 */
	public void setQuestion16(Integer[] question16) {
		this.question16 = question16;
	}

	/**
	 * @return the remarks16
	 */
	public String getRemarks16() {
		return remarks16;
	}

	/**
	 * @param remarks16
	 *            the remarks16 to set
	 */
	public void setRemarks16(String remarks16) {
		this.remarks16 = remarks16;
	}

	/**
	 * @return the question17
	 */
	public Integer[] getQuestion17() {
		return question17;
	}

	/**
	 * @param question17
	 *            the question17 to set
	 */
	public void setQuestion17(Integer[] question17) {
		this.question17 = question17;
	}

	/**
	 * @return the remarks17
	 */
	public String getRemarks17() {
		return remarks17;
	}

	/**
	 * @param remarks17
	 *            the remarks17 to set
	 */
	public void setRemarks17(String remarks17) {
		this.remarks17 = remarks17;
	}

	/**
	 * @return the question18
	 */
	public Integer[] getQuestion18() {
		return question18;
	}

	/**
	 * @param question18
	 *            the question18 to set
	 */
	public void setQuestion18(Integer[] question18) {
		this.question18 = question18;
	}

	/**
	 * @return the remarks18
	 */
	public String getRemarks18() {
		return remarks18;
	}

	/**
	 * @param remarks18
	 *            the remarks18 to set
	 */
	public void setRemarks18(String remarks18) {
		this.remarks18 = remarks18;
	}

	/**
	 * @return the question19
	 */
	public Integer[] getQuestion19() {
		return question19;
	}

	/**
	 * @param question19
	 *            the question19 to set
	 */
	public void setQuestion19(Integer[] question19) {
		this.question19 = question19;
	}

	/**
	 * @return the remarks19
	 */
	public String getRemarks19() {
		return remarks19;
	}

	/**
	 * @param remarks19
	 *            the remarks19 to set
	 */
	public void setRemarks19(String remarks19) {
		this.remarks19 = remarks19;
	}

	/**
	 * @return the question20
	 */
	public Integer[] getQuestion20() {
		return question20;
	}

	/**
	 * @param question20
	 *            the question20 to set
	 */
	public void setQuestion20(Integer[] question20) {
		this.question20 = question20;
	}

	/**
	 * @return the remarks20
	 */
	public String getRemarks20() {
		return remarks20;
	}

	/**
	 * @param remarks20
	 *            the remarks20 to set
	 */
	public void setRemarks20(String remarks20) {
		this.remarks20 = remarks20;
	}

	/**
	 * @return the question21
	 */
	public String getQuestion21() {
		return question21;
	}

	/**
	 * @param question21
	 *            the question21 to set
	 */
	public void setQuestion21(String question21) {
		this.question21 = question21;
	}

	/**
	 * @return the question22
	 */
	public String getQuestion22() {
		return question22;
	}

	/**
	 * @param question22
	 *            the question22 to set
	 */
	public void setQuestion22(String question22) {
		this.question22 = question22;
	}

	/**
	 * @return the question23
	 */
	public String getQuestion23() {
		return question23;
	}

	/**
	 * @param question23
	 *            the question23 to set
	 */
	public void setQuestion23(String question23) {
		this.question23 = question23;
	}

	/**
	 * @return the question24
	 */
	public String getQuestion24() {
		return question24;
	}

	/**
	 * @param question24
	 *            the question24 to set
	 */
	public void setQuestion24(String question24) {
		this.question24 = question24;
	}

	/**
	 * @return the question25
	 */
	public String getQuestion25() {
		return question25;
	}

	/**
	 * @param question25
	 *            the question25 to set
	 */
	public void setQuestion25(String question25) {
		this.question25 = question25;
	}

	/**
	 * @return the question11Db
	 */
	public Integer getQuestion11Db() {
		return question11Db;
	}

	/**
	 * @param question11Db the question11Db to set
	 */
	public void setQuestion11Db(Integer question11Db) {
		this.question11Db = question11Db;
	}

	/**
	 * @return the question12Db
	 */
	public Integer getQuestion12Db() {
		return question12Db;
	}

	/**
	 * @param question12Db the question12Db to set
	 */
	public void setQuestion12Db(Integer question12Db) {
		this.question12Db = question12Db;
	}

	/**
	 * @return the question13Db
	 */
	public Integer getQuestion13Db() {
		return question13Db;
	}

	/**
	 * @param question13Db the question13Db to set
	 */
	public void setQuestion13Db(Integer question13Db) {
		this.question13Db = question13Db;
	}

	/**
	 * @return the question14Db
	 */
	public Integer getQuestion14Db() {
		return question14Db;
	}

	/**
	 * @param question14Db the question14Db to set
	 */
	public void setQuestion14Db(Integer question14Db) {
		this.question14Db = question14Db;
	}

	/**
	 * @return the question15Db
	 */
	public Integer getQuestion15Db() {
		return question15Db;
	}

	/**
	 * @param question15Db the question15Db to set
	 */
	public void setQuestion15Db(Integer question15Db) {
		this.question15Db = question15Db;
	}

	/**
	 * @return the question16Db
	 */
	public Integer getQuestion16Db() {
		return question16Db;
	}

	/**
	 * @param question16Db the question16Db to set
	 */
	public void setQuestion16Db(Integer question16Db) {
		this.question16Db = question16Db;
	}

	/**
	 * @return the question17Db
	 */
	public Integer getQuestion17Db() {
		return question17Db;
	}

	/**
	 * @param question17Db the question17Db to set
	 */
	public void setQuestion17Db(Integer question17Db) {
		this.question17Db = question17Db;
	}

	/**
	 * @return the question18Db
	 */
	public Integer getQuestion18Db() {
		return question18Db;
	}

	/**
	 * @param question18Db the question18Db to set
	 */
	public void setQuestion18Db(Integer question18Db) {
		this.question18Db = question18Db;
	}

	/**
	 * @return the question19Db
	 */
	public Integer getQuestion19Db() {
		return question19Db;
	}

	/**
	 * @param question19Db the question19Db to set
	 */
	public void setQuestion19Db(Integer question19Db) {
		this.question19Db = question19Db;
	}

	/**
	 * @return the question20Db
	 */
	public Integer getQuestion20Db() {
		return question20Db;
	}

	/**
	 * @param question20Db the question20Db to set
	 */
	public void setQuestion20Db(Integer question20Db) {
		this.question20Db = question20Db;
	}

}
